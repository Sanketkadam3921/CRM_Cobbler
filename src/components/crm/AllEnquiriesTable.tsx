...
export function AllEnquiriesTable({ onBack }: AllEnquiriesTableProps) {
  ...
  const filteredEnquiries = allEnquiries.filter((enquiry) => {
    const matchesSearch =
      (enquiry.customerName || enquiry.name || "")
        .toLowerCase()
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in p-2 sm:p-0">
                  ...
                  <td className="p-3">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-xs text-foreground truncate">
                        {capitalizeFirst(enquiry.inquiryType)}
                      </span>
                    </div>
                  </td>
                  <td className="p-3">
                    <div className="flex items-center space-x-1">
                      <span className="text-xs text-foreground">
                        {capitalizeFirst(enquiry.product)}
                      </span>
                    </div>
                  </td>
                  <td className="p-3">
                    <div
                      className="text-sm text-foreground break-words line-clamp-2"
      ...
      <div className="lg:hidden space-y-4">
        {filteredEnquiries.map((enquiry) => (
          <Card
            key={enquiry.id}
            className="p-4 bg-gradient-card border-0 shadow-soft"
          >
                ...
                <div className="flex items-center space-x-1 text-gray-600">
                  ...
                  <span className="text-sm">
                    {enquiry.phone?.startsWith("+91")
                      ? enquiry.phone
                      : `+91 ${enquiry.phone || enquiry.number}`}
                  </span>
                </div>
